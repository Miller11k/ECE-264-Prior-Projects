...
ListNode *addListNode(ListNode **list, TreeNode *new_object, int (*cmpFunction)(TreeNode *, TreeNode *)){
    ...
    while(current != NULL){
        if(cmpFunction(current -> ptr, new_object) >= 0){break;}
        else{
            previous = current;
...
int treeNodeCompare(TreeNode *tp1, TreeNode *tp2){
    ...
    if(difference == 0){return((tp1 -> label) - (tp2 -> label));}
...
void printHuffmanTree(char *file, TreeNode *huffmanTree){
    ...
    if(ptr == NULL){fprintf(stderr, "No File for printHuffmanTree\n");}
...
void printHeader(char *file, TreeNode *huffmanTree){
    ...
    if(fptr == NULL){fprintf(stderr, "Error: Header File is NULL");}

    int current_count = 0;
    int current_byte = 0;

    printHeaderHelper(fptr, huffmanTree, &current_count, &current_byte);
    if(current_count == 0){fputc(0,fptr);}

    else{fputc(current_byte, fptr);}
    fclose(fptr);
}

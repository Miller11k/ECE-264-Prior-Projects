...
void interleave(CardDeck leftDeck, CardDeck rightDeck, int round)
{
  ...
  helper(var1, var2, (var1 + var2) + 1, leftDeck, rightDeck, combinedDeck, round);
  return;
}
...
void helper(int leftDeckIndex, int rightDeckIndex, int combinedIndex, CardDeck leftDeck, CardDeck rightDeck, CardDeck combinedDeck, int round)
{
  ...
  helper(leftDeckIndex - 1, rightDeckIndex, combinedIndex - 1, leftDeck, rightDeck, combinedDeck, round);
  
  combinedDeck.cards[combinedIndex] = rightDeck.cards[rightDeckIndex];
  helper(leftDeckIndex, rightDeckIndex - 1, combinedIndex - 1, leftDeck, rightDeck, combinedDeck, round);
}
...
{
    if (round == 0) // Immediately Check For Base Case
    {
        printDeck(origDeck);
        return;
    }

    // If not, run regularly
    ...
    {
        interleave(weave_left[i], weave_right[i], round);
    }
